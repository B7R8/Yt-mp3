...
const CryptoDonation: React.FC<CryptoDonationProps> = ({ navigateTo }) => {
  ...
  // Secure copy address to clipboard with verification
  const copyToClipboard = async (address: string, key: string) => {
    try {
      // Handle Binance User ID case (no network)
      if (key === 'binance') {
        // For Binance User ID, we don't need network verification
      ...
      const [crypto, network] = key.split('-');
      const success = await secureCopyToClipboard(address, crypto, network);
      
      if (success) {
        setCopiedAddress(key);
        setTimeout(() => setCopiedAddress(null), 2000);
      } else {
        console.error('Failed to copy address - verification failed');
      }
    } catch (error) {
