...
router.get('/admin/dashboard', async (req, res) => {
    try {
        ...
        const totalConversions = await database.get('SELECT COUNT(*) as count FROM conversions');
        // Get conversions by status
        const statusStats = await database.all('SELECT status, COUNT(*) as count FROM conversions GROUP BY status');
        // Get conversions from last 24 hours
        const last24Hours = await database.get(`SELECT COUNT(*) as count FROM conversions 
       WHERE created_at >= datetime('now', '-1 day')`);
        // Get conversions from last 7 days
        const last7Days = await database.get(`SELECT COUNT(*) as count FROM conversions 
       WHERE created_at >= datetime('now', '-7 days')`);
        // Get most popular video titles (top 10)
        const popularVideos = await database.all(`SELECT video_title, COUNT(*) as count 
       FROM conversions 
       ...
       LIMIT 10`);
        ...
        const qualityStats = await database.all(`SELECT quality_message, COUNT(*) as count 
       FROM conversions 
       WHERE quality_message IS NOT NULL
       GROUP BY quality_message`);
        // Get hourly distribution for today
        const hourlyStats = await database.all(`SELECT strftime('%H', created_at) as hour, COUNT(*) as count 
       FROM conversions 
       ...
       ORDER BY hour`);
        ...
        let fileCount = 0;
        try {
            const files = await fs_1.promises.readdir(downloadsDir);
            fileCount = files.length;
            for (const file of files) {
                const filePath = path_1.default.join(downloadsDir, file);
                const stats = await fs_1.promises.stat(filePath);
                diskUsage += stats.size;
            }
        }
        catch (error) {
            logger_1.default.warn('Could not read downloads directory:', error);
        }
        res.json({
            success: true,
            data: {
                ...
                statusStats: statusStats.reduce((acc, row) => {
                    acc[row.status] = row.count;
                    return acc;
                }, {}),
                popularVideos,
                qualityStats,
                hourlyStats: hourlyStats.reduce((acc, row) => {
...
router.get('/admin/jobs', async (req, res) => {
    try {
        ...
        // Get jobs
        const jobs = await database.all(`SELECT * FROM conversions 
       ${whereClause}
       ORDER BY created_at DESC 
       LIMIT ? OFFSET ?`, [...params, limit, offset]);
        // Get total count
        const totalResult = await database.get(`SELECT COUNT(*) as count FROM conversions ${whereClause}`, params);
        res.json({
            success: true,
            data: {
                jobs,
...
router.get('/admin/job/:id', async (req, res) => {
    try {
        const database = await database_1.db;
        const job = await database.get('SELECT * FROM conversions WHERE id = ?', [req.params.id]);
        if (!job) {
        ...
        if (job.mp3_filename) {
            try {
                const filePath = path_1.default.join(downloadsDir, job.mp3_filename);
                const stats = await fs_1.promises.stat(filePath);
                fileExists = true;
                fileSize = stats.size;
            }
        ...
        logger_1.default.error('Admin job details error:', error);
    ...
    try {
       ...
       WHERE status = 'completed' AND created_at < ?`, [cutoffTime.toISOString()]);
        ...
        for (const row of result) {
            if (row.mp3_filename) {
                const filePath = path_1.default.join(downloadsDir, row.mp3_filename);
                try {
                    const stats = await fs_1.promises.stat(filePath);
                    await fs_1.promises.unlink(filePath);
                    deletedFiles++;
                    deletedSize += stats.size;
                    logger_1.default.info(`Admin cleanup: Deleted file ${filePath}`);
                }
                catch (error) {
                    logger_1.default.warn(`Admin cleanup: Failed to delete file ${filePath}:`, error);
                }
            }
        ...
        logger_1.default.error('Admin cleanup error:', error);
    ...
    try {
        ...
        if (job.mp3_filename) {
            try {
                ...
                await fs_1.promises.unlink(filePath);
                fileDeleted = true;
                logger_1.default.info(`Admin delete: Deleted file ${filePath}`);
            }
            catch (error) {
                logger_1.default.warn(`Admin delete: Failed to delete file:`, error);
            }
        }
        ...
        logger_1.default.error('Admin delete job error:', error);
...
router.get('/admin/logs', async (req, res) => {
    try {
        const logFile = path_1.default.join(process.cwd(), 'logs', 'combined.log');
        try {
            const logContent = await fs_1.promises.readFile(logFile, 'utf-8');
        ...
        logger_1.default.error('Admin logs error:', error);
...
router.get('/admin/blacklist', async (req, res) => {
    try {
        ...
        const entries = await database.all(`SELECT * FROM blacklist 
       ${whereClause}
       ORDER BY created_at DESC 
       LIMIT ? OFFSET ?`, [...params, limit, offset]);
        // Get total count
        const totalResult = await database.get(`SELECT COUNT(*) as count FROM blacklist ${whereClause}`, params);
        res.json({
            success: true,
            data: {
                entries,
        ...
        logger_1.default.error('Admin blacklist error:', error);
...
router.post('/admin/blacklist', async (req, res) => {
    try {
        ...
        const existing = await database.get('SELECT id FROM blacklist WHERE type = ? AND value = ?', [type, value]);
        if (existing) {
            return res.status(400).json({
                success: false,
        ...
        // Add to blacklist
        const result = await database.run('INSERT INTO blacklist (type, value, reason, created_by) VALUES (?, ?, ?, ?)', [type, value, reason || null, 'admin']);
        res.json({
            success: true,
        ...
        logger_1.default.error('Admin add blacklist error:', error);
...
router.delete('/admin/blacklist/:id', async (req, res) => {
    try {
        const database = await database_1.db;
        ...
        const result = await database.run('DELETE FROM blacklist WHERE id = ?', [id]);
        if (result.changes === 0) {
        ...
        logger_1.default.error('Admin delete blacklist error:', error);
