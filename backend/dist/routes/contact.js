...
const createTransporter = () => {
    ...
    logger_1.default.error('No email configuration found. Please set up SendGrid, Mailgun, Gmail, or SMTP credentials.');
    return null;
};
// POST /api/contact - Send contact form email
router.post('/contact', rateLimiter_1.statusRateLimit, contactValidation, async (req, res) => {
    try {
...
You can reply directly to this email to respond to ${name}.
      `
        ...
        logger_1.default.info(`Contact form submitted successfully from ${email} (${name})`);
        res.json({
            success: true,
        ...
        logger_1.default.error('Contact form submission failed:', error);
        const userMessage = (0, errorHandler_1.getUserFriendlyError)(error);
        (0, errorHandler_1.logTechnicalError)(error, 'Contact Form Submission', req);
        res.status(500).json({
            success: false,
