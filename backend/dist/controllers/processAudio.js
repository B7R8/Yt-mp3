...
async function cleanupExpiredJobs() {
    try {
        ...
        try {
            ...
            if (cleanedCount > 0) {
                logger_1.default.info(`Legacy cleanup completed: ${cleanedCount} expired jobs removed`);
            }
        }
        catch (tableError) {
            // If table doesn't exist, skip cleanup gracefully
            if (tableError.code === '42P01' || tableError.message?.includes('does not exist')) {
                logger_1.default.info('Legacy jobs table does not exist, skipping cleanup');
                return;
            }
        ...
        logger_1.default.error('Legacy cleanup expired jobs failed:', error);
    }
}
